...
use Mike42\Escpos\PrintConnectors\WindowsPrintConnector;
    $date = date('d-M-Y H:i:s');  
        ...
        $printer -> initialize();
                  $testStr = ($cetak[0]['rezervasyonKodu']);
    $sizes = array(
     15 => "(maximum)");
    foreach ($sizes as $size => $label) {
    $printer -> setJustification(Printer::JUSTIFY_CENTER);
    $printer -> qrCode($testStr, Printer::QR_ECLEVEL_L, $size);
    $printer -> text($cetak[0]['rezervasyonKodu']);
    ...
    $printer -> feed(1);      
        ...
        $printer -> feed();
        $printer -> setJustification(Printer::JUSTIFY_LEFT);
        $printer -> text($i['biletKodu']);
        $printer -> feed();
        $printer -> text($i['saatKodu']   );
        $printer -> text("      ");
        $printer -> text($i['biletFiyat']);
        // $printer -> text("      ");        
        ...
        $printer -> setEmphasis(true);
        $printer -> text("Total     :              ");
        if (count($cetak) == '2') { $total = $cetak[0]['biletFiyat'] + $cetak[0]['biletFiyat'] ;;
                                            }else{ $total = $cetak[0]['biletFiyat'] ;}
        $printer -> text($total);
        $printer -> setEmphasis(false);
        $printer -> feed();
        $printer -> text("--------------------------------\n");
        ...
        $printer -> close();
    $printer -> close();
        redirect('tiket/tiketsaya/'.$cetak[0]['musteriKodu']);